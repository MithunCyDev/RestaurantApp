      ...
      <div className="pt-10 pb-4 px-6">
        ...
        >
          { cartItems && cartItems.map((items)=>(
            <div key={items.id} className="bg-dark h-24 rounded-md relative flex justify-between items-center mt-5">
            <motion.img
            ...
            <div className="flex flex-col">
              <p className="text-md text-white font-semibold">{items.title}</p>
              <p className="text-md text-green font-semibold">{items.price}</p>
            </div>
            <div className="flex items-center gap-3 px-4">
              <motion.div whileTap={{ scale: 0.7 }}>
                <AiOutlinePlus className="text-white text-sm cursor-pointer" />
              </motion.div>
              <p className="text-white text-lg font-semibold">{items.qty}</p>
              <motion.div whileTap={{ scale: 0.7 }}>
